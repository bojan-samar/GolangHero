...
const state = reactive({
    ...
    'deletingJobId': null,
    'deletingJobIndex': null,
})

const confirmJobDeletion = (jobId, index) => {
    state.deletingJobId = jobId;
    state.deletingJobIndex = index;
    state.confirmingJobDeletion = true;
};

const destroyJob = () => {
    axios.delete( route('admin.job.destroy', state.deletingJobId))
        .then(() => {
            props.jobs.data.splice(state.deletingJobIndex, 1)
...
<template>
    <AdminLayout title="Jobs Admin">
        <div class="max-w-4xl mx-auto">
                        ...
                        <TableRow v-for="(job, index) in jobs.data" :key="job.id">
                            ...
                            <TableCell>
                                <div class="flex items-center">
                                    <button class="bg-red-600 p-2.5 text-white rounded-full" @click="confirmJobDeletion(job.id, index)">
            ...
            <DialogModal :show="state.confirmingJobDeletion" @close="state.confirmingJobDeletion = false">
                <template #title>
                    Delete Job
                </template>
                ...
                <template #footer>
                    <SecondaryButton @click="state.confirmingJobDeletion = false">
                        Cancel
                    </SecondaryButton>

                    <DangerButton class="ml-3" @click="destroyJob">
                        Delete Alert
                    </DangerButton>
                </template>
            </DialogModal>
